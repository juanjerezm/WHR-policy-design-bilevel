...
def make_plot(collection):
    ...
    # Format x-axes
    if FORMATTED_XAXIS:
        major_ticks_primary = np.arange(-1500, 300 + 1e-3, 300)
        minor_ticks_primary = np.arange(-1500, 300 + 1e-3, 300)
        axs[-1].set_xlim(-1500, 300)
        axs[-1].set_xticks(major_ticks_primary)
        axs[-1].set_xticks(minor_ticks_primary, minor=True)
        axs[-1].set_xlabel("Change in carbon emissions (TWh/year)")
...
if __name__ == "__main__":
    ...
    PLOTNAME = "EmissionSummary-1"
    SAVE = False
    SHOW = True

    SCALE = 8760 / 73 * 1e-3 # Temporal scaling + ton to kton
    FIGSIZE = (8.5, 10)  # cm
    DPI = 600
    ...
    main(inputfile="ModelRuns-1.csv", var="x_h", multiplier="omega")
