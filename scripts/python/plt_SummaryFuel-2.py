...
def make_plot(collection):
    fig, axs = plt.subplots(
        len(collection.ratios_e),
        len(collection.ratios_c),
    ...
    # reverse order of ratios_e, to show increasing carbon reduction from top to bottom
    collection.ratios_e.reverse()
    collection.projects.reverse()

    # Plot each subplot and collect handles/labels
    for row, ratio_e in enumerate(collection.ratios_e):
        for col, ratio_c in enumerate(collection.ratios_c):
            ax = axs[row, col]
            bars, scatter = make_subplot(ax, collection, ratio_e, ratio_c)
...
def make_subplot(ax, collection, ratio_e, ratio_c):
    _, map_colors = mapping_values()

    df = pd.concat([run.results for run in collection.runs if run.ratio_e == ratio_e and run.ratio_c == ratio_c])
    df = df[["project", "F", "level"]]
    if INCLUDE_DEFICIT:
        df = pd.concat([df, chp_deficit(collection, ratio_e, ratio_c)])
    df = df.pivot(index="project", columns="F", values="level")
    df = df.reindex(collection.projects)  # This fills in missing rows with NaN
    if INCLUDE_DEFICIT and "Electricity (CPH-deficit)" in df.columns:
        df = df[[c for c in df.columns if c != "Electricity (CPH-deficit)"] + ["Electricity (CPH-deficit)"]]
    ...
    infeasible_records = [
        run for run in collection.runs if run.ratio_e == ratio_e and run.ratio_c == ratio_c and run.infeasible
    ]
    if infeasible_records:
        y_positions = [
            collection.projects.index(run.project) for run in infeasible_records
        ]
        nan = ax.scatter(
            [0] * len(y_positions),
            y_positions,
    ...
    # Format y-axes
    ax.set_ylabel(f"{ratio_e-100:+d}%", fontweight="bold", fontsize=10)
    ax.set_yticklabels([PROJECT_NAMES[proj] for proj in df.index])

    if ratio_e == collection.ratios_e[0]:
        ax.set_title(f"{ratio_c - 100:+d}%", fontweight="bold", fontsize=10)

    # Format x-axes
    if FORMATTED_XAXIS:
        major_ticks_primary = np.arange(X_RANGE[0], X_RANGE[1] + 1e-3, X_STEPS[0])
        minor_ticks_primary = np.arange(X_RANGE[0], X_RANGE[1] + 1e-3, X_STEPS[1])
...
def chp_deficit(collection, ratio_e, ratio_c):
    collection_copy = deepcopy(collection)
    for run in collection_copy.runs:
        if run.infeasible:
            continue
        run.load_result("x_h", "beta_b")
        run.results['J'] = "CHP_deficit"
        run.results['level'] = run.results['level'] * -1
        run.aggregate()
    df = pd.concat([run.results for run in collection_copy.runs if run.ratio_e == ratio_e and run.ratio_c == ratio_c])
    df = df[["project", "F", "level"]]
    return df
...
if __name__ == "__main__":
    ...
    if INCLUDE_DEFICIT:
        X_RANGE = (-10, 5)
        X_STEPS = (5, 2.5) # major and minor ticks
        X_LABEL = "Change in fuel consumption\n(TWh/year)"
    else:
        X_RANGE = (-10, 2)
        X_STEPS = (2, 1) # major and minor ticks
        X_LABEL = "Change in fuel consumption\n(TWh/year)"
    ...
    main(inputfile="ModelRuns-2.csv", var="x_h", multiplier="alpha")
